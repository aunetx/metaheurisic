...
class Algorithme:
    ...
    def iterer(self):
        ancienne_generation = [agent.copier().iterer() for agent in self.agents]
        ancienne_generation_selectionnes = selectionner_tranche(
            ancienne_generation,
            self.n_parents,
        ...
        enfants = []
        parents_selectionnes = np.random.choice(self.agents, size=2 * self.n_enfants, replace=False)
        for i in range(self.n_enfants):
            enfant = parents_selectionnes[
                i
            ].copier()  # enfanter(parents_selectionnes[2 * i], parents_selectionnes[2 * i + 1])
            enfant.muter()
            enfants.append(enfant)

        self.agents = ancienne_generation_selectionnes + enfants
